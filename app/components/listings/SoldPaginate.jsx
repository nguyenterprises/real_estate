...
function SoldPaginate({ properties }) {
  ...
  const totalPages = Math.ceil((soldProperties?.length - 1) / itemsPerPage);
  ...
  useEffect(() => {
    width >= 1024 ? setItemsPerPage(2) : setItemsPerPage(1);
  }, [width]);

  useEffect(() => {
    const svp = properties?.filter((status) => status.status == "sold");
    setSoldProperties(svp);
  }, [properties]);
            ...
            .map((listing, index) => (
              ...
              >
                <PreserveRatio
                  width="100%"
                  aspectRatio="6/9"
                  twStyles="rounded bg-[rgba(0,0,0,0.8)] text-white transition ease-in-out hover:scale-125 cursor-pointer brightness-50 hover:brightness-100"
                >
                  <RenderImage
                ...
                <div className="absolute bottom-3 left-3 text-white transition 2xl:flex 2xl:items-center 2xl:gap-[6rem] 2xl:pl-[1rem] 2xl:group-hover:hidden">
                  <div className="text-2xl font-medium 2xl:text-4xl">
                    ${new Intl.NumberFormat("en-US").format(listing.soldPrice)}
                  </div>
